...
var graphRef = database.ref("Average Time");
...
graphRef.on("value", function (snapshot) {
  var androidData = snapshot.child("Android").child("Data").val();
  var googleData = snapshot.child("Google").child("Data").val();

  chart.data.datasets[0].data = [androidData, googleData];
  chart.update();
});
